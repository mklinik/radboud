...
$S\_S\bot$.  Note that all finite terms end with either $0$ or $\bot$, and all
infinite terms are $\omega$-sequences of $S$ and $\_$.


\begin{defStrictOrderNuF}

Define the relation $\sqsubset$ on $\nu F \times \nu F$ as follows: two terms
$s$ and $t$ are in relation $s \sqsubset t$ iff $s$ is finite and $|s| \leq
|t|$ and $s_{|s|-1} = \bot$ and for all $i < n: s_i = t_i$
...
2, 3. The cases $x \neq y \wedge x \sqsubseteq y \wedge y = z$ and $x = y \wedge y
