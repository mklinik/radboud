...
The set $Z$ is a fixed point of the functor $F(X) = 1 + X + X$.

\end{thmDomNuFisFixedPoint}

\begin{proof}

We need to show that $Z$ is isomorphic to $F(Z)$, that is, provide two
functions $f : Z \arr F(Z)$ and $g : F(Z) \arr Z$ such that
...
The set $Z$ is the largest fixed point for F(X) = 1 + X + X.
