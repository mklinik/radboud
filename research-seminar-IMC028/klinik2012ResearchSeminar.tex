...
When looking at the subtree of $\_\bot$, we see that it has a similar structure
than the whole domain itself, except that every value is prefixed with one more
underscore.  Nodes in this subtree can be interpreted similarly as described
above, only that it took the process one internal computation step longer to
get to this point.
