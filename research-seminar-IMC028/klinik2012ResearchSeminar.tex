...
If it is clear from the context, we leave out the subscript in
$\semantics{-}_{\gamma}$ and just write $\semantics{-}$.


The informal idea of $(Z, \delta)$ encoding all possible
behaviours of $F$-coalgebras is made explicit in the following proof of TODO.
...
We need to prove two things.
  ...
  \begin{itemize}
  \item
    The function $\semantics{-}$ makes the diagram commute.
  \item
    The function $\semantics{-}$ is unique, i.e.~any other function $f : C \arr
    Z$ which makes the diagram commute, agrees with $\semantics{-}$ on every
    value of C.
  \end{itemize}
