...
// precise). UserEvents address the task by the label, while iTask Events
// address the task by an internal number.  toRealEvent translates between
// those two by querying the connection between task IDs and labels using the
// Refresh event.
