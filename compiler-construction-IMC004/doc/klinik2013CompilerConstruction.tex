...
gets type, say, $[\alpha]$, where $\alpha$ is fresh.  The trick is now to
generalize free variables of functions only if they are not free in the
environment.  In the example above, the type of $y$ is constrained to the type
of $x$, which has a free type variable, so the type of $f$ would be $\forall
\beta . ([\alpha] \arr \beta \arr \beta)$, where $\alpha$ is the same fresh type
variable used for the type of $x$.  This way, the type of the first argument of
$f$ and the type of $x$ are constrained together, which will give a type error
in one of the two calls of $f$ in \emph{main}.
...
The function \emph{id}, gets type $\forall \alpha .  (\alpha \arr \alpha)$, and
$f$ and $g$ both get type $\forall \alpha \beta . (\alpha \arr \alpha \arr
\beta)$.
...
become the same fresh type variables.  For example, if $(\alpha \arr \beta \arr
\alpha)$ is a type annotation, then \emph{derive}$(\alpha \arr \beta \arr
\alpha)$ returns $(\gamma \arr \delta \arr \gamma)$, with $\gamma$ and $\delta$
fresh.
