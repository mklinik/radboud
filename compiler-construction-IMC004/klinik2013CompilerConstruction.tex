...
function returns, it copies its return value to that place.
...
occupies one machine word, and the stack grows from bottom to top.
...
The following steps constitute a function call.  Because we are using a stack
machine, whenever an instruction needs to be given a non-constant parameter,
this parameter must be on top of the stack.  Depending on the order of doing
things when calling a function, some copying is required when returning.
...
compiler therefore generates the following code.
...
\begin{enumerate}
  ...
  \item The callee
    ...
    \item evaluates the initial value for each local variable and executes an
          assignment,
    \item and finally evaluates the function body.  The function body consists
    only of statements, which don't put values to the stack.  When the execution
    of the function body has finished, the callee
...
Maintaining access links for only one level of scoping seems overkill, so we
...
SPL supports functions as arguments to and return values from functions.  We
don't have nested functions, lambda expressions or partial function application.
Therefore, functions are stored on the stack as their address in memory.  When
calling a function, we use the \emph{jsr} instruction, which takes the address
to jump to from the stack.  This way, we can store functions in local variables
etc.
